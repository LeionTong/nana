...
less $file | awk '{upServer=$(NF);upTime=$(NF-1);if(upServer == "-"){upServer="Nginx"};if(upTime == "-"){upTime=0};upTimes[upServer]+=upTime;count[upServer]++;totalCount++;} END{for(server in upTimes){printf("%s %s%s %sms %s\n", count[server], count[server]/totalCount * 100, "%", 1000*upTimes[server]/count[server], server)}}' | sort -nr
